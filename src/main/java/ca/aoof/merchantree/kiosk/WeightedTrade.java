...
public class WeightedTrade implements IWeightedElement {
   public static final BuilderCodec<WeightedTrade> CODEC = BuilderCodec.builder(WeightedTrade.class, WeightedTrade::new)
      ...
      .add()
      .<PlayerItemStack>append(new KeyedCodec<>("Output", PlayerItemStack.CODEC), (wt, stack) -> wt.output = stack, wt -> wt.output)
      .addValidator(Validators.nonNull())
      .add()
      .<PlayerItemStack[]>append(
         new KeyedCodec<>("Input", new ArrayCodec<>(PlayerItemStack.CODEC, PlayerItemStack[]::new)), (wt, stacks) -> wt.input = stacks, wt -> wt.input
      )
      .addValidator(Validators.nonNull())
      .add()
   ...
   public WeightedTrade(double weight, @Nonnull PlayerItemStack output, @Nonnull PlayerItemStack[] input, int stock) {
      this.weight = weight;
      this.output = output;
      this.input = input;
   ...
   public WeightedTrade(double weight, @Nonnull PlayerItemStack output, @Nonnull PlayerItemStack[] input, int stockMin, int stockMax) {
      this.weight = weight;
      this.output = output;
      this.input = input;
   ...
   @Nonnull
   public PlayerTrade toPlayerTrade(@Nonnull Random random) {
